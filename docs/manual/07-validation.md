...
This chapter explains type checks, CFG checks, symbol checks, and backend preconditions. It is written for compiler engineers who already understand SSA, lowering, ABI boundaries, and pass pipelines, but who need a reliable map of Jiterati terminology and conventions.
...
- In the context of validation, define the boundary between generic Jiterati behavior and extension-specific behavior before adding new code.
...
- Validation should be understood as part of the larger Jiterati contract, not as an isolated implementation detail.
